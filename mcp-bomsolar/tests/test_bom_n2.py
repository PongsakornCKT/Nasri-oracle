...
def raw_fixture_2p5_extracts():
    ...
    # 6. MEA Fees from Labor & Fees tab: r[1] max_kw, r[2] fee (STRICT INDEX 2)
    mea_raw = {}
    for r in tabs.get("Labor & Fees", []):
        if isinstance(r, list) and len(r) > 2 and isinstance(r[0], (int, float)) and isinstance(r[1], (int, float)):
            mea_raw[int(r[1])] = _num(r[2])

    return inv_atmoce, sig_raw, mount_raw, cable_raw, panel_raw, mea_raw


def test_golden_1_atmoce21_10p_1p_2p5(raw_fixture_2p5_extracts):
    inv_atmoce, sig_raw, mount_raw, cable_raw, panel_raw, mea_raw = raw_fixture_2p5_extracts
...
def test_golden_mea_tier_30_20_to_30kw(raw_fixture_2p5_extracts):
    """Golden MEA Fee Tier 20<kW<=30 (12,500 THB) asserted against raw index 2 of Labor & Fees tab."""
    inv_atmoce, sig_raw, mount_raw, cable_raw, panel_raw, mea_raw = raw_fixture_2p5_extracts

    res = calculate_bom_n2(
        system="atmoce21",
        panels=40,  # 40 * 650W = 26.0 kWp -> 20 < kW <= 30
        phase="3P",
        fixture_filename="pricelist_fixture_2p5.json",
    )
    assert res["success"] is True
    line_map = {l["k"]: l for l in res["lines"]}
    assert line_map["D:meaFee"]["c"] == mea_raw[30]  # row[2] -> 12500.0
    assert line_map["D:meaFee"]["c"] == 12500.0


def test_golden_mea_tier_40_30_to_40kw(raw_fixture_2p5_extracts):
    """Golden MEA Fee Tier 30<kW<=40 (15,500 THB) asserted against raw index 2 of Labor & Fees tab."""
    inv_atmoce, sig_raw, mount_raw, cable_raw, panel_raw, mea_raw = raw_fixture_2p5_extracts

    res = calculate_bom_n2(
        system="atmoce21",
        panels=50,  # 50 * 650W = 32.5 kWp -> 30 < kW <= 40
        phase="3P",
        fixture_filename="pricelist_fixture_2p5.json",
    )
    assert res["success"] is True
    line_map = {l["k"]: l for l in res["lines"]}
    assert line_map["D:meaFee"]["c"] == mea_raw[40]  # row[2] -> 15500.0
    assert line_map["D:meaFee"]["c"] == 15500.0
