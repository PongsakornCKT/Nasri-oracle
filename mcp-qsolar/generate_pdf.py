...
import re
import sys as _sys
_QSOLAR_DIR = os.path.dirname(os.path.abspath(__file__))
_PARENT_DIR = os.path.dirname(_QSOLAR_DIR)
...
SELLING_PRICES = {
    # ATMOCE residential range synced to the Finalprice tab 2026-08-11
    # (keys = panels = W/625, matching get_selling_price's convention).
    # Small (2-3 panels), 1P 24 and the 3P C&I tail (≥24) keep the previous
    # values — Finalprice doesn't cover them.  Old intermediate keys inside
    # the covered range were dropped so _nearest_key rounds up to the next
    # defined package instead of hitting a stale price.
    'ATMOCE': {
        '1P': {
            2: 89000, 3: 100000, 4: 123000, 6: 163000, 8: 190000,
            10: 225000, 12: 255000, 16: 285000, 20: 319000, 24: 419000,
        },
        '3P': {
            8: 210000, 12: 245000, 16: 280000, 18: 310000, 20: 349000,
            24: 479000, 30: 569000, 32: 569000, 36: 629000, 40: 759000,
            42: 759000, 50: 890000, 60: 980000,
        },
    },
    'Huawei': {
...
ATMOCE_IMAGES = {
    ...
    # 2:1 microinverter variants (MI-1250 + AIKO 670W, 2 panels per micro)
    '1P_onGrid_2to1':    [_pic('1Phase-Atmoce-21.png'),                 _pic('Atmoce-system.jpg'), _pic('ตัวอย่างการติดตั้งบนหลังคา.jpg'), _pic('certificate.jpg')],
    '1P_batt_2to1':      [_pic('1Phase-batt-Atmoce-21.png'),            _pic('Atmoce-system.jpg'), _pic('ตัวอย่างการติดตั้งบนหลังคา.jpg'), _pic('certificate.jpg')],
    '1P_batt_bkup_2to1': [_pic('1Phase-batt-Atmoce-fullsystem-21.png'), _pic('Atmoce-system.jpg'), _pic('ตัวอย่างการติดตั้งบนหลังคา.jpg'), _pic('certificate.jpg')],
    '3P_onGrid_2to1':    [_pic('3Phase-Atmoce-21.png'),                 _pic('Atmoce-system.jpg'), _pic('ตัวอย่างการติดตั้งบนหลังคา.jpg'), _pic('certificate.jpg')],
    '3P_batt_2to1':      [_pic('3Phase-batt-Atmoce-21.png'),            _pic('Atmoce-system.jpg'), _pic('ตัวอย่างการติดตั้งบนหลังคา.jpg'), _pic('certificate.jpg')],
    '3P_batt_bkup_2to1': [_pic('3Phase-batt-Atmoce-fullsystem-21.png'), _pic('Atmoce-system.jpg'), _pic('ตัวอย่างการติดตั้งบนหลังคา.jpg'), _pic('certificate.jpg')],
}
SIGENERGY_IMAGES = [
    ...
    try:
        ...
        if live_phase:
            if brand == 'ATMOCE':
                ...
                lookup = round(size_kw * 1000 / pw)
            else:
                lookup = int(size_kw)
            key = _nearest_key(live_phase, lookup)
            # Only trust live data when it can round UP to a covering
            # package.  _nearest_key falls back to the LAST key for
            # oversize requests — the live table may not carry the C&I
            # tail (e.g. ATMOCE 3P ≥24 panels), and quoting a 30 kW
            # system at the 12.5 kW package price must never happen.
            # The hardcoded fallback below still has those sizes.
            if key >= lookup:
...
def _atmoce_battery_lines(phase: str, has_backup: bool, batt_qty: int = 1) -> list:
    ...
    qty_prefix = f'จำนวน : {batt_qty} เครื่อง  ' if batt_qty > 1 else ''
    backup_model = 'MU100S' if phase == '1P' else 'MU100T'
    backup_phase_label = '1P' if phase == '1P' else '3P'
    backup_phase_th = '1 เฟส' if phase == '1P' else '3 เฟส'

    if has_backup:
        title = f'MS-7K (ESS) + พร้อมระบบสำรองไฟ (Backup System) {phase_label}'
        lines = [
            (FB, 13, title),
            (F, 11, f'1. 7kw. Extra low voltage [ELV] Energy Storage System ({phase_text})'),
            (F, 11, f'BATTERY ENERGY STORAGE SYSTEM {qty_prefix}MS-7K-U(ประกัน 10ปี) แบตเตอรี่ลิเธียมฟอสเฟต (LFP)'),
            (F, 11, 'แรงดันต่ำ (Extra Low Voltage หรือ ELV) ความปลอดภัยสูง อายุการใช้งาน 10,000 รอบการชาร์จ'),
            (F, 11, f'2. ตู้ ATMOCE M-Backup Box {backup_phase_label} ({backup_model}) สำหรับระบบไฟฟ้า {backup_phase_th} 1ชุด'),
...
def _inverter_section_lines(brand: str, phase: str, size_kw: float, model: str,
                            ...
                            panel_brand: str = '', panel_watt: int = 0,
                            micro_2to1: bool = False) -> list:
    ...
    micro_2to1: ATMOCE only — force MI-1250 2:1 micro layout (25-yr warranty).
    ...
    spec = get_spec(brand, phase, size_kw, micro_2to1=micro_2to1)
...
def _warranty_lines(brand: str, micro_2to1: bool = False) -> list:
    panel_years = '15' if brand == 'ATMOCE' else '12'
    inverter_years = INVERTER_WARRANTY_YEARS.get(brand, '10')
    # ATMOCE 2:1 (MI-1250 micro) — 25-year microinverter warranty.
    if brand == 'ATMOCE' and micro_2to1:
        inverter_years = '25'
        inverter_label = 'Microinverter ATMOCE MI-1250'
    else:
        inverter_label = f'INVERTER ยี่ห้อ {brand}'
    lines = [
        ...
        (F, 11, f'- {inverter_label} รับประกัน {inverter_years} ปี'),
...
def _terms_page1_lines(brand: str, grand_total: float, payment_rounds: int = 2) -> list:
    header = [
    ...
    if payment_rounds == 3:
        ...
        c = grand_total * 0.40
        payment = [
            (F, 11, 'การชำระเงินแบ่ง ออกเป็น 3 รอบ'),
            (F, 11, f'งวดแรก 30% : หลังเซ็นสัญญา ({fmt(a)} บาท)'),
            (F, 11, f'งวดที่สอง 30% : หลังสินค้าเข้าไปถึงหน้างาน ({fmt(b)} บาท)'),
            (F, 11, f'งวดที่สาม 40% : หลังเสร็จสิ้นงาน ({fmt(c)} บาท)'),
        ]
    else:
        # Default 2 rounds: 60% / 40% split
        dep = grand_total * 0.60
        fin = grand_total * 0.40
        payment = [
            (F, 11, 'การชำระเงินแบ่ง ออกเป็น 2 รอบ'),
            (F, 11, f'รอบที่ 1 - 60% Payment With PO ({fmt(dep)} บาท)'),
            (F, 11, f'รอบที่ 2 - 40% หลังจากติดตั้งเรียบร้อย ({fmt(fin)} บาท)'),
        ]
    tail = [
    ...
    return header + payment + tail


def _terms_page2_lines(brand: str, micro_2to1: bool = False) -> list:
    inverter_years = INVERTER_WARRANTY_YEARS.get(brand, '10')
    # ATMOCE 2:1 micro (MI-1250) — 25-yr microinverter warranty.
    if brand == 'ATMOCE' and micro_2to1:
        inverter_years = '25'
...
class QuotationGenerator:
    ...
    def generate(self, data: dict) -> str:
        ...
        salesperson  = data.get('salesperson', 'จันทิมา นาเดช')
        ...
        if not output_path:
            ...
            if battery_only:
                ...
                if has_backup:
                    safe_name += '_bkup'
            filename = f'{quote_number}_{safe_name}'
            # raw_customer: trust caller's customer name unless empty / placeholder.
            # Defensive: also strip non-printable + replacement chars (U+FFFD, zero-width)
            # to prevent garbage suffixes when upstream encoding/parsing went sideways.
            _candidate = (customer or '').strip() if customer else ''
            if _candidate and _candidate != 'ใบเสนอราคา':
                raw_customer = _candidate
            else:
                raw_customer = 'ใบเสนอราคา Enervia'
            # Drop filesystem-unsafe chars + control chars + replacement/zero-width
            safe_customer = re.sub(r'[\\/:*?"<>|\x00-\x1f\x7f​-‍﻿�]+', '', raw_customer).strip()
            safe_customer = re.sub(r'\s+', '_', safe_customer)
            # Skip suffix unless name has at least one Thai/Latin/digit char.
            # Stronger than the previous `re.fullmatch(r'_+', ...)` check: now
            # also rejects "_._.", "...", "—__", and any combination of
            # punctuation/underscores/separators with no real letter content.
            if safe_customer and re.search(r'[฀-๿A-Za-z0-9]', safe_customer):
                filename += f'-{safe_customer}'
        ...
        for _pass_key in ('panel_brand', 'panel_watt', 'panel_count', 'battery_model', 'battery_kwh', 'has_optimizer', 'credit', 'battery_only', 'micro_2to1', 'payment_rounds'):
    ...
    def _build_pdf(self, filename: str, d: dict):
        ...
        micro_2to1 = bool(d.get('micro_2to1', False))
        images = self._get_images(brand, phase, has_battery, has_backup, micro_2to1)
        ...
        terms2 = _terms_page2_lines(brand, micro_2to1=bool(d.get('micro_2to1', False)))
        cy = y - 10
    ...
    def _build_items(self, brand, panels, phase, size_kw, has_battery, has_backup, grand_total, data=None, lump_sum=False):
        ...
        if not inverter_model:
            if brand == 'Huawei':
                ...
                _snapped = sigenergy_snap_size(size_kw, phase)
        ...
        if not battery_only:
            ...
            inv_lines = _inverter_section_lines(brand, phase, size_kw, inverter_model,
                                                ...
                                                micro_2to1=bool(data.get('micro_2to1', False)) if data else False)
        ...
        _micro_2to1 = bool(data.get('micro_2to1', False)) if data else False
        items.append((item_num, _warranty_lines(brand, micro_2to1=_micro_2to1), 0.0, 0.0))
        _payment_rounds = int(data.get('payment_rounds', 2)) if data else 2
        items.append((item_num + 1, _terms_page1_lines(brand, grand_total, payment_rounds=_payment_rounds), 0.0, 0.0))
    ...
    # ──────────────────────────────────────────────────────────
    def _get_images(self, brand, phase, has_battery, has_backup, micro_2to1=False):
        if brand == 'ATMOCE':
    ...
    def _header(self, c, d) -> float:
        ...
        for label, value in [
            ...
            (_lbl['date'],        d['date']),
            (_lbl['credit'],      d.get('credit', '3 วัน')),
    ...
    def _summary_bar(self, c, y: float, d: dict) -> float:
        ...
        # Inverter label
        if brand == 'ATMOCE':
            _micro_2to1 = bool(d.get('micro_2to1', False))
            if _micro_2to1 or (phase == '3P' and size_kw >= 30):
                inv_label = 'MI-1250'
            else:
                inv_label = 'MI-500'
    ...
    def _financial_summary(self, c, cy, grand_total, brand, size_kw, discount=0.0, remarks=None,
                           markup_pct=0.0, base_price=0.0, micro_2to1=False):
        ...
        # ATMOCE 2:1 (MI-1250) only: include free bird-mesh installation note.
        if brand == 'ATMOCE' and micro_2to1:
            std_notes.append('- แถมฟรี: ตะแกรงกันนก พร้อมติดตั้ง')
