...
def _validate_generate_inputs(
    ...
    if battery_only:
        # battery-only quotes: size_kw=0 is valid, but still cap upper bound
        if not (0 <= size_kw <= 10000):
            raise ValueError(f"size_kw must be between 0 and 10000, got {size_kw}")
    else:
...
def qsolar_generate(
    ...
    battery_only : When True, skip solar panel (Section 1) and inverter (Section 2) rendering.
                   size_kw=0 is accepted. grand_total must be > 0. Use for standalone battery
                   quotations (e.g. ATMOCE MS-7K, Sigenergy, Dyness) without any PV system.
    ...
    try:
        ...
        if grand_total <= 0:
            if battery_only:
                # battery-only quotes MUST have grand_total > 0 (no panel/inverter to auto-calc)
                return {'success': False, 'error': 'grand_total must be > 0 for battery_only quotes'}
        ...
        # battery-only: force has_battery=True, override project_name/output_path if not set
        if battery_only:
            has_battery = True
            if not project_name:
                bkwh_label = f'{battery_kwh:.4g}kWh ' if battery_kwh > 0 else ''
                project_name = f'Battery {bkwh_label}{phase} {brand}'
            if not safe_path:
                os.makedirs(_gp_module.OUTPUT_DIR if _gp_module else '/tmp', exist_ok=True)
                _out_dir = _gp_module.OUTPUT_DIR if _gp_module else '/tmp'
                os.makedirs(_out_dir, exist_ok=True)
                safe_name = f'{brand}_batt-only_{phase}'
                if battery_kwh > 0:
                    safe_name += f'_{battery_kwh:.4g}kWh'
                safe_path = os.path.join(_out_dir, f'{quote_number}_{safe_name}.pdf')
