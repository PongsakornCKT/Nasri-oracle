...
function calculateKeenocMounting(catalog, panelQty, roofType) {
  ...
  var railInfo = searchKeenoc('4200', 340.0);
  if (!railInfo.row) railInfo = searchKeenoc('Rail', 340.0);
  items.push({
    part_number: 'RAIL-4200',
    part_name: railInfo.name || 'Keenoc Aluminum Rail 4200mm (รางยึดแผง 4.2m)',
    manufacturer: 'Keenoc',
  ...
  var ecInfo = searchKeenoc('End Clamp', 12.50);
  if (!ecInfo.row) ecInfo = searchKeenoc('End', 12.50);
  items.push({
    part_number: 'END-CLAMP',
    part_name: ecInfo.name || 'Keenoc End Clamp (ตัวล็อคริมแผง 30/35mm)',
    manufacturer: 'Keenoc',
  ...
  if (mcQty > 0) {
    var mcInfo = searchKeenoc('Mid Clamp', 12.50);
    if (!mcInfo.row) mcInfo = searchKeenoc('Mid', 12.50);
    items.push({
      part_number: 'MID-CLAMP',
      part_name: mcInfo.name || 'Keenoc Mid Clamp (ตัวล็อคกลางแผง 30/35mm)',
      manufacturer: 'Keenoc',
      category: 'mounting_clamp',
      quantity: mcQty,
      unit: 'ตัว',
  ...
  items.push({
    ...
    notes: '2 ตัว/แผง สำหรับ ' + roofType,
    price_source: hookInfo.source
  });

  // 5. Grounding Lug — panelQty * 1
  var lugInfo = searchKeenoc('Grounding Lug', 13.95);
  items.push({
    part_number: 'GND-LUG',
    part_name: lugInfo.name || 'Keenoc Grounding Lug (กราวด์ลัก)',
    manufacturer: 'Keenoc',
    category: 'mounting_accessory',
    quantity: panelQty,
    unit: 'ตัว',
    unit_cost: lugInfo.price,
    total_cost: panelQty * lugInfo.price,
    notes: '1 ตัว/แผง',
    price_source: lugInfo.source
  });

  // 6. Earthing Clip — panelQty * 2
  var clipInfo = searchKeenoc('Earthing Clip', 4.80);
  items.push({
    part_number: 'EARTH-CLIP',
    part_name: clipInfo.name || 'Keenoc Earthing Clip (แผ่นกราวด์คลิป)',
    manufacturer: 'Keenoc',
    category: 'mounting_accessory',
    quantity: panelQty * 2,
    unit: 'ตัว',
    unit_cost: clipInfo.price,
    total_cost: panelQty * 2 * clipInfo.price,
    notes: '2 ตัว/แผง',
    price_source: clipInfo.source
  });

  // 7. Cable Clip — panelQty * 5
  var ccInfo = searchKeenoc('Cable Clip', 4.80);
  items.push({
    part_number: 'CABLE-CLIP',
    part_name: ccInfo.name || 'Keenoc Cable Clip (คลิปรัดสายไฟ)',
    manufacturer: 'Keenoc',
    category: 'mounting_accessory',
    quantity: panelQty * 5,
    unit: 'ตัว',
    unit_cost: ccInfo.price,
    total_cost: panelQty * 5 * ccInfo.price,
    notes: '5 ตัว/แผง',
    price_source: ccInfo.source
  });
