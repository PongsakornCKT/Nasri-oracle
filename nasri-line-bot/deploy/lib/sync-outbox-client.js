 ...
 *
 * sobek Phase 07: HMAC-SHA256 anti-replay signing.
 *   sig = HMAC-SHA256(timestamp + body, LF_BOT_SECRET)
 *   headers: X-LF-Signature, X-LF-Timestamp
 * Signed AT FLUSH TIME, per attempt — NOT at enqueue time. A row can sit in
 * the outbox across retries (backoff up to 30min, longer across a process
 * restart); a signature minted once at enqueue and resent on every retry is
 * itself a replayable artifact once observed. Every retry gets a fresh
 * timestamp + signature. (Lesson from sekhmet's earlier outbox: sign-once-
 * at-enqueue meant a captured request could be replayed indefinitely.)
 * WP-side verification spec: ψ/active/nasri-oa-v2/APPLY.md (isis to implement).
 *
...
/** sig = HMAC-SHA256(timestamp + body, secret) — exported for test determinism. */
...
module.exports = function createSyncOutboxClient(opts) {
  ...
  async function flushOutbox() {
    ...
    for (var i = 0; i < pendingRows.length; i++) {
      ...
      try {
        // sobek P07: sign THIS attempt, fresh timestamp every time — never reuse
        // a signature minted at enqueue or on an earlier retry.
        var _ts = Date.now();
        var _sig = signRequest(botSecret, _ts, row.payload_json);
  ...
  return {
    ...
    signRequest: signRequest, // exposed for tests
    db: db
  };
};
