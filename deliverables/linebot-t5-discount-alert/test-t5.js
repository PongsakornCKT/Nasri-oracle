/**
 * test-t5.js — Verification Test Suite for T5v2 Discount Alert (#B5 - REVISION 2)
...
async function runTests() {
  console.log('🧪 [T5v2 Test] Starting test suite...');
  ...
  // Selling = 100,000, Normal Profit = 20,000 (20%), Discount = 5,000 -> Net Profit = 15,000 (15.0%)
  const okRes = alertEngine.checkDiscountImpact(5000, 20000, 100000);
  assert.strictEqual(okRes.isViolation, false, 'Discount leaving 15.0% margin should not trigger alert');
  ...
  // Selling = 100,000, Normal Profit = 15,000 (15%), Discount = 8,000 -> Net Profit = 7,000 (7.0%)
  const violationRes = alertEngine.checkDiscountImpact(8000, 15000, 100000);
  assert.strictEqual(violationRes.isViolation, true, 'Discount leaving 7.0% margin MUST trigger violation');
  assert.strictEqual(violationRes.finalMarginPct, 7.0, 'Final net margin should be 7.0%');
  assert.ok(violationRes.warningText.includes('ส่วนลด ฿8,000 ส่งผลให้มาร์จิ้นเหลือ 7.0%'), 'Warning text should be formatted for sales');
  ...
  console.log('✅ [T5v2 Test] All 3 assertion checks PASSED cleanly!');
}

runTests().catch(err => {
  console.error('❌ [T5v2 Test] FAILED:', err);
  process.exit(1);
});
